...
def fetch_test_data():
    test_file = open('./new_testing.txt')
...
def data_fetch():
    ...
    falling_training_num = int(falling_labels.shape[1]* 0.8 )
...
def knearestneigbour():
    ...
    new_testing_data = fetch_test_data()
    new_testing_rf_prediction = rf_classifer.predict(new_testing_data[0].reshape(1,-1))

    return int(new_testing_rf_prediction  )       #int(rf_prediction[0])
